...
const UserDetails = () => {
    ...
    const updateMetaTags = (personalInfo) => {
        ...
        const metaTags = [
            { property: 'og:title', content: `${personalInfo.first_name} ${personalInfo.last_name}` },
            { property: 'og:description', content: personalInfo.summary || 'User Profile' },
            { property: 'og:image', content: personalInfo.profile_picture_url },
            { property: 'og:url', content: window.location.href },
            { property: 'og:type', content: 'profile' },
            { name: 'twitter:card', content: 'summary_large_image' },
            { name: 'twitter:title', content: `${personalInfo.first_name} ${personalInfo.last_name}` },
            { name: 'twitter:description', content: personalInfo.summary || 'User Profile' },
  ...
  if (error) return (
    ...
    const { personalInfo, educations, experiences, projects, certifications } = userData;
                        ...
                        <div className="flex flex-col sm:flex-row">
                            ...
                            <div className="flex-1 mt-4 sm:mt-2">
                                ...
                                <p className="text-gray-500 text-sm mt-1">
                                    {personalInfo?.address?.city?.name}  {personalInfo?.address?.state?.name}  {personalInfo?.address?.country?.name}
                                    {/* <span className="text-blue-600 ml-1">Contact info</span> */}
                                </p>

                                <div className="flex flex-wrap items-center mt-3 text-sm text-gray-500">
                                    <span className="mr-4">{personalInfo.connection_count} connections</span>
                                    <span className="mr-4">{personalInfo.follower_count} followers</span>
                                ...
                                <div className="flex flex-wrap gap-2 mt-4">
                                    {personalInfo?.frame_status !== 'none' && (
                                        <button className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-1.5 px-4 rounded-full text-sm">
                                            {personalInfo?.frame_status === 'none' ? "" : personalInfo?.frame_status}
                                        </button>
                            ...
                            {experiences.map((exp, index) => (
                                <div key={index} className="mb-6 last:mb-0">
                                    <div className="flex">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-900">{exp.profileName}</h3>
                                            <p className="text-gray-700">{exp.companyName}</p>
                                            <p className="text-gray-500 text-sm">
                                                {formatDate(exp.start_date)} - {formatDate(exp.end_date)}
                                            </p>
                                            <p className="text-gray-500 text-sm">{exp.industryName}</p>
                                            {exp.description && (
                                                <p className="text-gray-700 mt-2">{exp.description}</p>
                                            )}
                                            {exp.skills_acquired && exp.skills_acquired.length > 0 && (
                                                <div className="mt-3 flex flex-wrap gap-2">
                                                    {exp.skills_acquired.map(skill => (
                                                        <span key={skill._id} className="bg-gray-100 text-gray-700 text-xs px-2.5 py-1 rounded-full">
                                                            {skill.name}
                                                        </span>
                                                    ))}
                                                </div>
                                ...
                                <div key={index} className="mb-6 last:mb-0">
                                    <div className="flex">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-700">{edu.degree} - {edu.field_of_studies}</p>
                                            <p className="text-gray-500 text-sm">
                                            ...
                                            {edu.skills_acquired && edu.skills_acquired.length > 0 && (
                                                <div className="mt-3 flex flex-wrap gap-2">
                                ...
                                {projects.map((project, index) => (
                                    <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
                                        {isImage(project?.media_url) ? (
                                            ...
                                            <iframe
                                                src={`https://docs.google.com/gview?url=${encodeURIComponent(project?.media_url)}&embedded=true`}
                                                title="file-preview"
                                        ...
                                        <div className="p-4">
                                            ...
                                            {project.institution && (
                                                <div className="flex justify-start gap-2 items-center ">
                                                    ...
                                                    <p className="text-xs text-gray-500 font-medium">{project.institution}</p>
                                                </div>
                                            )}
                                            {project.file_url && (
                                                <a
                                ...
                                <div key={index} className="mb-6 last:mb-0">
                                    <div className="flex">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-sm">Issued {formatDate(cert.issue_date)}</p>
                                            {cert.credential_id && (
                                                <p className="text-gray-500 text-sm">Credential ID {cert.credential_id}</p>
                                            )}
                                            {cert.credential_url && (
                                                <a
