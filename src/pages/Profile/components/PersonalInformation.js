...
const PersonalInformation = ({ formData, error, isExtended, setIsExtended, setFormData, stateList, citiesList, loading, handleChange, countryList, handleSelectChange, handleSubmit, setErrors }) => {
    ...
    <div className="flex flex-col gap-6 bg-white border rounded-2xl">
      <div className="p-8 rounded-lg">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold text-[#000000E6]">Personal Info</h3>
          ...
          {!isExtended ? (
            ...
            <Button variant="zinc" rounded="full" icon={<BiLeftArrowAlt />} onClick={() => setIsExtended((prev) => !prev)}>
              Back
            </Button>
          )}
        </div>
        ...
        <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isExtended ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className={`pt-6 ${isExtended ? 'block' : 'hidden'}`}>
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-4 items-end lg:grid-cols-4 md:grid-cols-2 mt-6">
              <FilterSelect2
                label="Gender"
                selectedOption={selectJson?.gender?.find(opt => opt.value === getFormValue('gender')) || null}
                onChange={(option) => {
            ...
            <div className="flex items-center justify-end mt-6 gap-3">
              ...
              <Button variant='ghost' onClick={() => setIsExtended((prev) => !prev)} >
                Close
              </Button>
