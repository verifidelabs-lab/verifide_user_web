...
const Users = () => {
    ...
    const { suggestedUserData: { data }, loading, error } = selector || {};
    ...
    const handleConnect = async (data) => {
        switch (activeTab) {
            case 'user':
                navigate(`/user/profile/${data?.first_name || data?.username}/${data?._id}`);
                break;
        ...
        switch (activeTab) {
            case 'user':
                try {
                    ...
                    dispatch(suggestedUser({ page: 1, size: usersPerPage, type: activeTab }));
                } catch (error) {
                    toast.error(error);
                } finally {
    ...
    return (
        ...
        ${activeTab === tab.key
            ...
            <div className="mb-8 flex md:flex-row flex-col justify-between items-center">
            ...
            {!loading && !error && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-2 justify-items-center">
                    {items.map(item => (
                        <UserProfileCard
                            key={item._id}
            ...
            {!loading && !error && items.length === 0 && (
                <div className="text-center py-16">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">
                        No {tabConfig.find(t => t.key === activeTab)?.label.toLowerCase()} found
                    </h3>
                    <p className="text-gray-500 max-w-md mx-auto">
                        {activeTab === 'user' && "No user suggestions available right now. Check back later for connection opportunities."}
                        {activeTab === 'companies' && "No companies to explore at the moment. New companies are added regularly."}
                        {activeTab === 'institutions' && "No institutions available currently. Educational partners are being added soon."}
                    </p>
                </div>
            )}
