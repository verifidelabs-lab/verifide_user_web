...
import { fetchLoginCredentials, login, registerWithGoogle } from "../../redux/slices/authSlice";
...
const Login = () => {
  ...
  const handleGoogleSubmit = async (e) => {
    ...
    try {
      const response = await dispatch(
        registerWithGoogle({ redisToken: googleLoginObject.passkey, username: formData.username, password: formData.password, confirmPassword: formData.confirmPassword })
      ).unwrap();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        >
          {googleLoginObject.event !== 'new_comer' ? <>
            <Form onSubmit={handleSubmit} className="w-full max-w-md px-4 sm:px-0">
              ...
              {[
                ...
                <div className="flex items-center space-x-3 p-4 bg-yellow-50 rounded-lg shadow-sm mt-4">
                  ...
                  <p className="text-base text-gray-700 font-medium">{googleLoginObject.message}</p>
                </div>
              }
            ...
            <>
              <Form onSubmit={handleGoogleSubmit} className="w-full max-w-md px-4 sm:px-0">
                <div
                  className="leading-relaxed tracking-wider"
                  data-aos="fade-up"
                  data-aos-delay="100"
