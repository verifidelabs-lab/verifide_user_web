...
const WorkExperience = () => {
  ...
  const allCompanies = arrayTransform2(
    selector?.getAllCompaniesData?.data?.data || []
  );
  const allIndustry = arrayTransform(
    selector?.getAllIndustryData?.data?.data || []
  );
  const allProfile = arrayTransform(
    selector?.getAllProfileRoleData?.data?.data || []
  );
  const allWorkSkill = arrayTransform(
    selector?.getAllWorkSkillListData?.data?.data || []
  );

  // Transform skills for suggestions
  const allSkills =
    selector?.getAllWorkSkillListData?.data?.data?.map((skill) => ({
      value: skill?._id,
      label: skill?.name,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submissionData = {
        ...
        duration: formData.currently_working
          ? null
          : getDuration(formData.start_date, formData.end_date),
        skills_acquired: selectedSkills.map(
          (skill) => skill.value || skill._id
        ),
      };
  ...
  const handleSelectChange = async (field, selected) => {
    ...
    try {
      if (field === "company_id" && selected?.value) {
        ...
        await dispatch(
          getAllWorkSkillList({
            profile_role_id: selected.value,
            created_by_users: selected?.created_by_users || null,
          })
        );
      }
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="flex justify-between flex-wrap place-items-center gap-4 py-5">
                <Button
                  ...
                  onClick={() =>
                    navigate(
                      `/education-details?redirect=${encodeURIComponent(
                        redirectUrl
                      )}`
                    )
                  }
