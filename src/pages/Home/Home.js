...
import { PiDotsThreeOutlineVerticalFill, PiHeartStraightFill, PiHeartStraightLight, PiShareFat, PiWarning } from 'react-icons/pi';
import { BsExclamationCircle } from 'react-icons/bs';
import { IoCopyOutline } from 'react-icons/io5';
import { AiOutlineDelete } from 'react-icons/ai';
import { useDispatch, useSelector } from 'react-redux';
import {
  commentOnPost, getPostOnHome, likeDislikePostComment, suggestedUser, replyToComment,
  getCommentOnPost, messageChatUser, deletePost, reportPost, followUnfollowUsers, updatePostLike, deleteComment,
...
const Home = () => {
  ...
  const observeNext = useIO({
    onIntersect: () => {
      if (hasMoreNext && !isLoadingRef.current && !endOfContent) fetchPosts(currentPage + 1, 'next');
    },
    rootMargin: '200px',
    threshold: 0.1,
  });
  ...
  useEffect(() => {
    if (lastPostRef.current && hasMoreNext && !isLoading) observeNext(lastPostRef.current);
  }, [posts, hasMoreNext, isLoading, observeNext]);

  useEffect(() => {
    if (firstPostRef.current && hasMorePrev && !isLoading && currentPage > 1) observePrev(firstPostRef.current);
  ...
  const handleLike = useCallback(
    async (postId) => {
      ...
      setLoadingStates((p) => ({ ...p, likes: { ...p.likes, [postId]: true } }));
      ...
      try {
            ...
            prevPosts.map(post =>
              post._id === postId
                ? {
                  ...
                  isLiked: response.data.isLiked !== undefined ? response.data.isLiked : newLikedState
                }
                : post
            )
          );
        ...
        setLoadingStates((p) => ({ ...p, likes: { ...p.likes, [postId]: false } }));
      }
    },
  ...
  const handleDelete = async (commentId, postId) => {
    ...
    try {
      ...
      //   }
      // }));
      const res1 = await dispatch(deleteComment({ comment_id: commentId })).unwrap()
  ...
  const handleFollowClick = useCallback(
    async (userId, userPath) => {
      setLoadingStates((p) => ({ ...p, follows: { ...p.follows, [userId]: true } }));
      try {
        await dispatch(
          followUnfollowUsers({
            target_id: userId,
        ...
        setLoadingStates((p) => ({ ...p, follows: { ...p.follows, [userId]: false } }));
      }
    },
  ...
  const handleCommentSubmit = useCallback(
    async (postId, text) => {
      ...
      setLoadingStates((p) => ({ ...p, comments: { ...p.comments, [postId]: true } }));
      ...
      try {
        ...
        setLoadingStates((p) => ({ ...p, comments: { ...p.comments, [postId]: false } }));
      }
    },
  ...
  const handleReplySubmit = useCallback(
    async (parentCommentId, text) => {
      try {
        await dispatch(replyToComment({ parentComment: parentCommentId, text })).unwrap();
      } catch {
  ...
  const handleCommentLike = useCallback(
    async (contentId, contentType) => {
      try {
        await dispatch(likeDislikePostComment({ content_id: contentId, content_type: contentType })).unwrap();
      } catch {
  ...
  const toggleComments = useCallback(async (postId) => {
    setActiveCommentSections((prev) => ({ ...prev, [postId]: !prev[postId] }));
  ...
  const handleCopyLink = useCallback((post) => {
    if (post && post?._id) {
      const baseUrl = `https://dev-verifide.verifide.xyz/postView/${post?._id}`;
  ...
  const handleReportSubmit = useCallback(
    async (type, reason) => {
      ...
      setLoadingStates((p) => ({ ...p, reports: { ...p.reports, [selectedPostForReport]: true } }));
      try {
        await dispatch(reportPost({ post_id: selectedPostForReport, type, reason })).unwrap();
        ...
        setLoadingStates((p) => ({ ...p, reports: { ...p.reports, [selectedPostForReport]: false } }));
      }
    },
  ...
  const handleDeletePost = useCallback(async () => {
    ...
    setLoadingStates((p) => ({ ...p, deletes: { ...p.deletes, [postIdToDelete]: true } }));
    try {
      ...
      setLoadingStates((p) => ({ ...p, deletes: { ...p.deletes, [postIdToDelete]: false } }));
    }
  ...
  console.log('2222222222222', commentsData["68c506de8c890beb0a1c427c"] || [], commentsData);
      ...
      <div className="flex flex-col md:flex-row w-full mx-auto gap-4">
        <div className="xl:w-[75%] lg:w-[70%] md:w-[60%] w-full space-y-6 overflow-hidden h-screen  overflow-y-auto   hide-scrollbar">
          <div className="flex justify-between items-center gap-2 mb-2 text-sm">
            ...
            <FilterDropdown tabs={tabs} tabActive={tabActive} setTabActive={setTabActive} />
          </div>
          ...
          {Array.isArray(posts) && posts.map((post, index) => {
            ...
            return (
              <>
                {index === 4 && suggestedUsers?.data?.list && suggestedUsers?.data?.list.length > 0 && (
                  <div className='md:hidden block'>
                    ...
                    <SuggestedUsersSwiper
                      ...
                      onExploreMore={() => navigate(`/user/suggested-users`)}
                    />
                  </div>
                )}
                <div
                  ...
                  ref={isFirst ? firstPostRef : isLast ? lastPostRef : undefined}>
                  <div className="flex items-start justify-between mb-4">
                    <button
                      ...
                      onClick={() => {
                        if (post.userData?.user_path !== 'Companies') {
                          navigate(
                            `/user/profile/${post.userData?.first_name || post.userData?.name}/${post.userData?._id}`
                          );
                        } else {
                          toast.info('You cant view companies');
                        }
                      }}
                      title="View profile">
                      {post?.userData?.profile_picture_url ? (
                        <img
                          src={post?.userData?.profile_picture_url || '/0684456b-aa2b-4631-86f7-93ceaf33303c.png'}
                          alt={`${post?.userData?.first_name || post?.userData?.name} ${post?.userData?.last_name || ''}`}
                          className="object-cover rounded-full w-12 h-12 border"
                          onError={(e) => {
                            const fallback = '/0684456b-aa2b-4631-86f7-93ceaf33303c.png';
                      ...
                      <div className="text-left">
                        <h3 className="md:text-lg text-md font-semibold text-[#000000E6] capitalize hover:text-gray-700">
                          {post.userData?.first_name || post?.userData?.name} {post.userData?.last_name}
                        </h3>
                        <p className="md:text-sm text-xs text-gray-600">
                          {post.userData?.headline ||
                            (post.userData?.user_path === "Users" ? "user" : post?.userData?.user_path)}
                        </p>
                      </div>
                    </button>
                    ...
                    <div className="flex items-center">
                      {!post?.isSelfPost && (
                        <FollowButton isFollowing={post?.isConnected}
                          isLoading={loadingStates.follows[post.userData?._id]}
                          onClick={() => handleFollowClick(post.userData?._id, post.userData?.user_path)}
                        />
                      ...
                      <div className="relative ml-2" >
                        ...
                        {showOptionsDropdown === post._id && (
                          ...
                          >
                            ...
                            {(post?.post_type === 'certificates' || post?.post_type === 'jobs') ? <></> :
                              <button
                  ...
                  {Array.isArray(post?.tags) && post.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                      {post.tags.map((tag, i) => (
                        <span key={i} className="md:text-sm text-sm font-semibold text-[#6B6B6B] capitalize">
                          #{tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    <ActionButton
                      icon={localLikes[post._id] ? PiHeartStraightFill : PiHeartStraightLight}
                    ...
                    <ActionButtonComment
                      ...
                      ariaLabel="Comments"
                    />
                    <ActionButton icon={PiShareFat} onClick={() => handleShareClick(post)} ariaLabel="Share" />
                  </div>
          ...
          {isLoading && (
            <div className="space-y-4 mt-2">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="animate-pulse bg-gray-100 p-6 rounded-xl h-36" />
              ))}
            </div>
          )}
        </div>
