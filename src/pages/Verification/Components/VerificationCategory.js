...
import { verificationCenterDocumentDetails, verifyRequest } from '../../../redux/Verification/Verification';
import { instituteCollegeList } from '../../../redux/education/educationSlice';
import { adminUsersDetails } from '../../../redux/Users/userSlice';
import { arrayTransform, convertTimestampToDate, getDuration, uploadImageDirectly, uploadPdfDirectly } from '../../../components/utils/globalFunction';
...
const VerificationCategory = ({ profileData }) => {
  ...
  const fetchInstituteData = useCallback(async (institutionId) => {
    if (!institutionId) return;

    try {
      const res = await dispatch(adminUsersDetails({
  ...
  const handleOptionClick = useCallback(async (option, itemId, item) => {
    const newOption = activeOption === option && selectedItemId2 === itemId ? null : option;
    setActiveOption(newOption);
    setSelectedItemId2(itemId);

    // console.log("option, itemId, item", item?.company_id)

    if (newOption === 'institution' || (item?.institution_id || item?.company_id)) {
  ...
  const handleFileUpload = useCallback(async (uploadType, file) => {
    ...
    try {
      const action = file.type === "application/pdf" ? uploadPdfDirectly : uploadImageDirectly
  ...
  const handleInstitutionSubmit = useCallback(async (e, data) => {
    e.preventDefault();
    ...
    const payload = institutionForm.institute_id === ''
      ? {
        type: DOCUMENT_MODELS[type],
        document_id: data?._id,
    ...
    try {
      ...
      toast.success(res?.message || 'Institution verification submitted successfully');
      await fetchDataByTab();
      resetForms();
    } catch (error) {
      toast.error(error?.message || "Request failed or already submitted");
  ...
  }, [dispatch, formData.image_url, institutionForm, type, fetchDataByTab, resetForms]);
  ...
  const renderStatusBadge = (status) => {
    ...
    return (
      <span className={`text-sm px-2 py-1 rounded-full border capitalize ${statusClasses[status] || statusClasses.pending}`}>
        {status === "pending" ? "UnVerified" : status === "approved" ? "Verified" : status}
      </span>
    );
  };
  ...
  const renderInstitutionForm = (item) => (
    <form onSubmit={(e) => handleInstitutionSubmit(e, item)} className="p-4 space-y-4 bg-blue-50 rounded-lg">
      <FilterSelect
        options={allInstituteList}
        selectedOption={allInstituteList.find(opt => opt.value === institutionForm.institute_id)}
        onChange={(select) => handleSelectChange("institute_id", select)} label='Select Verified'
      />
      ...
      <div className="flex justify-end space-x-2">
        <Button variant='zinc' type="button" onClick={() => setActiveOption(null)}>
          Cancel
        </Button>
        <Button variant='warning' type="button" onClick={() => handlePreview(item)}
          disabled={!institutionForm?.email && !institutionForm?.name && !formData?.image_url}>
          Preview
        </Button>
  ...
  const renderDocumentForm = (item) => (
    <form onSubmit={(e) => handleSubmit(e, item)} className="p-4 space-y-4 bg-blue-50 rounded-lg">
    ...
    <div className="min-h-screen  bg-[#F6FAFD]  p-4">
      <div className="w-full mx-auto">
        <nav className="flex items-center py-4 text-sm">
          ...
          <span className="text-gray-600 hover:text-[#000000E6]" onClick={() => window.history.back()}>{formattedTab(tab)}</span>
          <BiChevronRight className="w-4 h-4 text-gray-400 mx-2" />
          <span className="text-blue-600 font-medium">
            {type ? `${type.charAt(0).toUpperCase() + type.slice(1)} Verification` : 'Verification'}
          </span>
        </nav>
      </div>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 p-4 bg-[#FFFFFF] rounded-lg ">
        ...
        <ul className="space-y-4">
          {apiRes && apiRes.length > 0 ? (
            apiRes.map((item) => (
              <li key={item._id || item.id} className='border-[#00000030]/20 border rounded-xl'>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center text-sm text-gray-600 space-x-4">
                          <span>{item.institution || item.profileName || item.issuing_organization || item?.description}</span>
                ...
                >
                  <div className="space-y-4 my-4 p-2 rounded-lg ">
                    {item?.verificationHistory?.map((history) => (
                      <div key={history._id} className="p-4 border rounded-lg shadow-sm bg-white transition-all duration-300 hover:shadow-md">
                        ...
                        <div className="text-sm text-gray-600 mb-2">
                          ...
                          {history.is_verified_by_third_person ? (
                            <p>Third-party verifier - <span className="font-medium">{history.third_person_name}</span> ({history.third_person_email})</p>
                          ) : (
                                  ...
                                  >
                                    {isImage ? (
                                      ...
                                      <div className="flex flex-col items-center justify-center p-3 h-24 text-center">
                                        ...
                                        <p className="mt-1 text-xs text-gray-600 truncate">PDF Document</p>
                                      </div>
                                    ) : (
                                      <div className="flex flex-col items-center justify-center p-3 h-24 text-center">
                                        <svg
                                        ...
                                        <p className="mt-1 text-xs text-gray-600 truncate">Attachment</p>
                                      </div>
                                    )}
                                  </a>
                                );
                ...
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${selectedItemId === item._id ? 'max-h-[1000px]' : 'max-h-0'
                  }`}>
                      ...
                      <div className='border border-[#C3D6FF] rounded-lg'>
                        ...
                        >
                          <div className='flex justify-start items-center gap-3'>
                            ...
                            <div>
                              <p className='text-[#000000] md:text-xl text-lg font-semibold'>Verification by {item?.company_id ? "Company" : "Institute"}</p>
                        ...
                        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${activeOption === 'institution' && selectedItemId2 === item._id
                    ...
                    <div className='border border-[#C3D6FF] rounded-lg'>
                      ...
                      >
                        <div className='flex justify-start items-center gap-3'>
                          ...
                          <div>
                            <p className='text-[#000000] md:text-xl text-lg font-semibold'>Verification by Document Upload</p>
                            <p className='text-[#6B6B6B] md:text-sm text-xs font-normal'>
                      ...
                      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${activeOption === 'document' && selectedItemId2 === item._id
                        ? 'max-h-[500px]'
                        : 'max-h-0'
                        }`}>
      ...
      <Modal isOpen={isPreview} title={`Preview the mail`} onClose={() => { setIsPreview(false); setMailData({}) }}
