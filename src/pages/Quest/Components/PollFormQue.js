...
export const PollFormQue = ({
  ...
  index // question index to match errors
}) => {
  const [isEditing, setIsEditing] = useState(false);
          ...
          <div className="space-y-2 mt-2">
            {question.options.map((option, optIndex) => (
              <div key={optIndex} className="flex flex-col">
                <div className="flex items-center">
                  {["poll", "multi-choice", "checkbox"].includes(question.type) && (
                    <input
                      ...
                      onChange={(e) => {
                        if (question.type === "checkbox") {
                          ...
                          else newValue.splice(newValue.indexOf(option), 1);
                          onResponseChange(newValue);
                        } else {
                ...
                {errors?.[`surveyOption-${index}-${optIndex}`] && (
    ...
    <div className="border rounded-lg p-4 mb-6 transition-colors">
      <div className="flex justify-between items-start mb-4 gap-2">
          ...
          {isEditing ? (
            <>
              <input
                ...
                onChange={(e) => onTitleChange(e.target.value)}
                onBlur={() => setIsEditing(false)}
                className={`flex-1 min-w-0 text-sm font-medium p-2 border-b glassy-input focus:outline-none focus:border-blue-500 break-words breaka-all truncate ${errors?.[`surveyQuestion-${index}`] ? "border-red-500" : ""
                  }`}
                autoFocus
              />
              {errors?.[`surveyQuestion-${index}`] && (
            ...
            <>
              <h2
                className="text-sm font-medium cursor-text p-2 glassy-text-secondary rounded border truncate break-words breaka-all"
                onClick={() => setIsEditing(true)}
        ...
        <select
          value={question.type}
          onChange={(e) => onTypeChange(e.target.value)}
          className="border rounded p-1 text-sm glassy-select glassy-text-secondary flex-shrink-0 w-36"
        >
          <option value="short-answer">Short answer</option>
          <option value="poll">Poll</option>
          <option value="multi-choice">Multiple choice</option>
