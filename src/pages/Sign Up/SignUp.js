...
import { getCookie, setCookie, removeCookie } from '../../components/utils/cookieHandler'
...
const SignUp = () => {
    ...
    const handleOtpSubmit = async (e) => {
        ...
        try {
            ...
            const otpValue = otp.join('')
            const res = await dispatch(verifyRegisterOtp({
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            toast.error(error?.message || error || 'Failed to resend verification code. Please try again.')
        } finally {
            setIsSubmitting(false)
        }
    }
                                    ...
                                    <div className="flex justify-center mb-4" onPaste={handlePaste}>
                                        {Array.from({ length: 6 }).map((_, index) => (
                                            <input
                                                ...
                                                disabled={isSubmitting}
                                                className={`w-12 h-12 mx-1 text-center text-xl bg-[#FFFFFF] border-2 rounded-lg focus:ring-2 focus:ring-blue-300/30 outline-none transition-all duration-200 ${otpError
                                    ...
                                    <div className="text-center mb-4">
                                        <p className="text-gray-500 text-sm">
                                            ...
                                            >
                                                ...
                                                {isResendDisabled && remainingTime > 0 && ` (${formatTime(remainingTime)})`}
                                            </button>
                                        </p>
                                    </div>
