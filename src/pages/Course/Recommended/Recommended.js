...
const Recommended = () => {
  ...
  const coursecategoryList = [
    { value: "", label: "All" },
    ...arrayTransform(
      globalSelector?.getAllCourseCategoryData?.data?.data?.list
    ),
  ];
  ...
  const handleBookmark = async (courseId) => {
    try {
      const res = await dispatch(
        addBookmarked({ course_id: courseId })
      ).unwrap();
      setCourses(
        courses.map((course) =>
          course._id === courseId
  ...
  return (
    <div className=" flex flex-col md:flex-row justify-between gap-3   h-[90vh] p-2 overflow-hidden">
      ...
      <div className="md:hidden md:mb-4">
        <div
          className="flex border-b border-gray-200 overflow-x-auto gl whitespace-nowrap no-scrollbar 
               -mx-4 px-4"
        >
          {tab?.map((item, index) => {
            const isActive = item === activeSection;
            ...
            <ul className="space-y-2">
              {tab?.map((item, index) => {
                ...
                if (item < activeSection) {
                  ...
                  textColor =
                    "text-blue-600  bg-[#2563EB1A]/10 text-[#2563EB]  rounded";
                }

                return (
                  <li
                    key={index}
                    className={`xl:text-base lg:text-sm md:text-sm font-normal p-2 xl:w-52  lg:w-40 md:w-40  flex justify-start items-center gap-2 capitalize ${textColor} ${
                      activeSection ? " " : ""
      ...
      <div className="w-full flex-1 p-1 md:p-6 mx-auto h-screen custom-scrollbar overflow-hidden overflow-y-auto">
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row flex-wrap items-center justify-between gap-4">
            <h1 className="text-xl md:text-2xl font-semibold glassy-text-primary">
