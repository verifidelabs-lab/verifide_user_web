    ...
    <div className="flex items-center gap-2">
      <div className="flex justify-start items-center gap-2">
        {course?.organization_logo_url && (
          <img
            src={course?.organization_logo_url || "/Placeholder 2.png"}
            alt="logo"
            onError={(e) => {
        ...
        <p className="glassy-text-primary text-xs">
...
const CourseCard = ({ course, handleCourseDetails, onBookmarkToggle }) => {
  return (
    ...
    // >
    <div
      className="rounded-xl relative glassy-card hover:shadow-lg transition-shadow 
             duration-300 p-3 cursor-pointer group min-h-[430px] flex flex-col justify-between"
    >
      ...
      <div className="pt-4 px-1 relative">
        ...
        <p className="text-sm glassy-text-secondary line-clamp-2 mb-2 min-h-[40px]">
          {course?.description}
        </p>
        ...
        <div className="min-h-12">
          {course?.skills?.map((e) => {
            return (
              <>
                <span className="glassy-card glassy-text-primary rounded-full px-2 py-0.5 border text-[10px] ">
                  {e?.name}
                </span>
              </>
            );
