...
import { getAllDegree, getAllFieldsOfStudy, getAllSkillList, instituteCollegeList, updateDataCompany, updateDegreeData, updateFieldsOfStudyData, updateSkillsData } from '../../redux/education/educationSlice'
import { arrayTransform, convertToTimestamp, getDuration } from '../../components/utils/globalFunction'
...
import { updateCompanyData, updateIndustryData, updateProfileRoleData } from '../../redux/work/workSlice'
...
const EducationDetails = () => {
    ...
    const collegeList = arrayTransform(selector?.instituteCollegeListData?.data?.data || [])
    const degreeList = arrayTransform(selector?.getAllDegreeData?.data?.data || [])
    const fieldsOfStudyList = arrayTransform(selector?.getAllFieldsOfStudyData?.data?.data || [])
    const skillsList = arrayTransform(selector?.getAllSkillListData?.data?.data || [])
    ...
    const { formData, setFormData, handleChange, errors, setErrors } = useFormHandler({
        "institution_id": "",
        "degree_id": "",
        "field_of_studies": "",
        "start_date": "",
        ...
        if (field === "institution_id") {
            ...
            if (selected?.value) {
                ...
                try {
                    ...
                    await dispatch(getAllDegree({ institution_id: selected.value, created_by_users: selected?.created_by_users || null }));
                } catch (error) {
            ...
            if (selected?.value) {
                ...
                try {
                    ...
                    await dispatch(getAllFieldsOfStudy({ degree_id: selected.value, created_by_users: selected?.created_by_users || null }));
                } catch (error) {
            ...
            if (selected?.value) {
                ...
                try {
                    ...
                    await dispatch(getAllSkillList({ study_id: selected.value, created_by_users: selected?.created_by_users || null }));
                } catch (error) {
            ...
            const skillsArray = Array.isArray(selected) ? selected : (selected ? [selected] : [])
    ...
    const handleAddItem = async () => {
        ...
        try {
            ...
            switch (addModalState.field) {
                ...
                case 'degree_id':
                    if (formData.institution_id) {
                        dispatch(getAllDegree({ institution_id: formData.institution_id, created_by_users: isCreatedByUser || null }));
                    }
                    break;
                case 'field_of_studies':
                    if (formData.degree_id) {
                        dispatch(getAllFieldsOfStudy({ degree_id: formData.degree_id, created_by_users: isCreatedByUserForFields || null }));
                    }
                    break;
                case 'skills_acquired':
                    if (formData.field_of_studies) {
                        dispatch(getAllSkillList({ study_id: formData.field_of_studies, created_by_users: isCreatedByUserForFields || null }));
                    }
                    break;
            }
                        ...
                        <div className="w-full max-w-lg border-[0.5px] bg-[#FFFFFF] border-[#A9A9A9]/50 shadow-sm rounded-[10px] p-4" data-aos="fade-left">
                            ...
                            <form className="space-y-4" onSubmit={handleSubmit}>
                                <FilterSelect
                                    ...
                                    selectedOption={collegeList?.find(opt => opt.value === formData?.institution_id) || null}
                                    options={collegeList || []}
                                ...
                                <FilterSelect
                                    ...
                                    options={degreeList || []}
                                    selectedOption={degreeList?.find(opt => opt.value === formData?.degree_id) || null}
                                ...
                                <FilterSelect
                                    ...
                                    selectedOption={fieldsOfStudyList?.find(opt => opt.value === formData?.field_of_studies) || null}
                                    onChange={(selected) => handleSelectChange("field_of_studies", selected)}
                                ...
                                <div className='max-w-lg overflow-hidden'>
                                    ...
                                    {errors.skills_acquired && (
                                        <p className="text-red-500 text-xs mt-1">{errors.skills_acquired}</p>
                                    )}
                                </div>
