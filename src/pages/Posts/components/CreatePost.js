...
const CreatePost = () => {
    ...
    async (selectedFiles) => {
      ...
      try {
        ...
        if (imageFiles.length > 0) {
          const result = await uploadMultiImageDirectly(
            imageFiles,
            "POST_MEDIA"
          );
        ...
        if (videoFiles.length > 0) {
          ...
          if (videoFiles.length > 1) {
            toast.info(
              "Only one video can be uploaded. Using the first selected video."
            );
          }
          let result;
          try {
            result = await uploadVideoDirectly(videoFiles[0], "POST_MEDIA");
          } catch (err) {
  ...
  const handleRemoveUploadedFile = useCallback((fileUrl, type) => {
    if (type === "image") {
      setPostData((prev) => ({
        ...prev,
        image_urls: prev.image_urls.filter((url) => url !== fileUrl),
      }));
    } else if (type === "video") {
      setPostData((prev) => ({
        ...prev,
  ...
  const handleFileUpload2 = async (file) => {
    ...
    setIsLoading(true);
    try {
      const result = await uploadVideoDirectly(file, "POST_MEDIA");
  ...
  const handleFileUpload = useCallback(
    async (file) => {
      ...
      try {
        if (allowedImageTypes.includes(file.type)) {
          const result = await uploadImageDirectly(file, "POST_MEDIA");
  ...
  return (
    <div className="create-post-container min-h-screen p-4">
        ...
        <div className=" overflow-hidden space-y-3 p-4">
          ...
          <form className=" space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              {renderError("title")}
              <div
                className={`text-sm text-right ${getCharacterCountColor(
              ...
              <div className="space-y-3">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-400 transition-colors">
                  <CustomImageVideoUpload
                    ...
                    maxSizeMB={5}
                    isLoading={isLoading}
                    removeUploadedFile={handleRemoveUploadedFile} // Add this prop
            ...
            {postData.post_type === "link" && (
              <div className="space-y-3">
                ...
                <CustomInput
                  ...
                  className={`w-full p-4 transition-all duration-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent rounded-lg border-2 ${
                    errors.link
                      ? "border-red-500 bg-red-50"
                      : "border-gray-200 hover:border-gray-300"
                  }`}
                ...
                <p className="text-sm text-gray-600">
                  Share an interesting link with your audience
                </p>
            ...
            {postData.post_type === "poll" && (
              <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600">
                    How long should the poll be open for voting? (1-30 days)
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600">
                    {postData.poll.options.length}/10 options • Add at least 2
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={
                    postData.tags.length >= 10 || !postData.tagInput.trim()
                  }
                >
                  Add
                </button>
              ...
              {postData.tags.length > 0 && (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500">
                    {postData.tags.length}/10 tags • Press Enter or click Add to
                    add tags
                  </p>
                </div>
              )}
            ...
            <div className="flex justify-end pt-6 border-t border-gray-200">
              <button
                ...
                className={`px-8 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-300 font-medium flex items-center shadow-lg hover:shadow-xl ${
                  isSubmitting
