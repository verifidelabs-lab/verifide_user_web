    ...
    return (
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col items-center justify-center h-72 gap-3 animate-pulse">
          <NoDataFound
            message={
              "User details not found. Please log in again or contact support."
            }
          />
        </div>
      </div>
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
          ...
          <button
            onClick={downloadPDF}
            disabled={isGeneratingPDF}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 glassy-text-primary font-medium py-2 px-4 rounded-md transition-colors disabled:opacity-50"
          >
            {isGeneratingPDF ? (
              <>
                <div className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
        ...
        <div className="min-h-screen py-10 flex justify-center">
          <div
            ref={resumeRef}
            className="w-full max-w-4xl glassy-card shadow-xl grid grid-cols-1 md:grid-cols-3 rounded-lg overflow-hidden"
          >
            {/* Left Sidebar */}
            <div className="glassy-card p-6 flex flex-col gap-8">
              {/* Profile Image */}
              <div className="flex justify-center">
                <img
                  src={
                  ...
                  alt="profile"
                  className="w-28 h-28 md:w-32 md:h-32 rounded-full border-4 border-yellow-500 object-cover"
                  onError={(e) => {
              ...
              {/* Contact */}
              <div className="glassy-text-primary">
                <h2 className="text-xl font-semibold mb-3 border-b border-yellow-400 pb-1">
                  Contact Me
                </h2>

                <p className="mb-2">
                  📞 +{profileData?.personalInfo?.country_code?.dial_code}{" "}
                  {profileData?.personalInfo?.phone_number}
                </p>

                <p className="mb-2 break-all">
                  ✉️ {profileData?.personalInfo.email}
                </p>

                <p className="break-words whitespace-normal glassy-text-primary">
                  📍 {profileData?.personalInfo?.address?.address_line_1}
                  {profileData?.personalInfo?.address?.address_line_2 &&
                    `, ${profileData.personalInfo.address.address_line_2}`}
                  {profileData?.personalInfo?.address?.city?.name &&
                    `, ${profileData.personalInfo.address.city.name}`}
                  {profileData?.personalInfo?.address?.state?.name &&
                    `, ${profileData.personalInfo.address.state.name}`}
                  {profileData?.personalInfo?.address?.pin_code &&
                    ` - ${profileData.personalInfo.address.pin_code}`}
                  {profileData?.personalInfo?.address?.country?.name &&
                    `, ${profileData.personalInfo.address.country.name}`}
                </p>
              </div>
              ...
              {/* Education */}
              {Array.isArray(profileData.educations) &&
                      ...
                      {profileData.educations.map((edu, index) => (
                        <li key={index}>
                          <p className="font-medium text-lg break-words">
                            {capitalizeWords(edu.institution)}
                          </p>
                          <p className="text-sm">
                            {capitalizeWords(edu.degree)} (
                            {formatDateByMomentTimeZone(edu.start_date, "YYYY")}{" "}
            ...
            {/* Right Content */}
            <div className="col-span-2 p-6 md:p-10 flex flex-col gap-10">
              {/* Header */}
              <div className="glassy-text-primary" >
                <h1 className="text-2xl md:text-3xl font-bold capitalize break-words">
                  {profileData.personalInfo.first_name}{" "}
                  {profileData.personalInfo.last_name}
                </h1>
                <p className="text-lg glassy-text-secondary break-words">
              ...
              <div className="glassy-text-primary" >
                ...
                <p className="break-words glassy-text-secondary leading-relaxed">
                  {profileData.personalInfo.summary}
                </p>
              </div>
              ...
              {profileData?.topSkills?.data?.length > 0 && (
                <div>
                  <h2 className="text-2xl md:text-3xl glassy-text-primary font-semibold text-yellow-500 mb-4">
                    Skills
                  </h2>

                  <div className="grid grid-cols-2 gap-2 glassy-text-secondary">
                    {profileData.topSkills.data.map((skill) => (
                      <span key={skill.skill_id}>✔ {skill.skill_name}</span>
                    ))}
                  </div>
