...
export const uploadMultiImageDirectly = async (files, moduleName = "DEFAULT") => {
    ...
    try {
        const response = await axiosImage.post("/global-module/upload-multi", formData, {
            headers: {
...
export const uploadPdfDirectly = async (file, moduleName = 'DEFAULT') => {
    ...
    try {
        const response = await axiosImage.post('/global-module/upload-doc', formData);
        return response.data;
    } catch (error) {
        console.error('Upload error:', error);
        throw error;
...
export const uploadVideoDirectly = async (file, moduleName = 'DEFAULT') => {
    ...
    try {
        const response = await axiosImage.post('/global-module/upload-video', formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
...
const generateResumePDF = (data) => {
    try {
        ...
        // Helper functions
        const addText = (text, x, y, fontSize = 10, color = textColor, fontStyle = 'normal') => {
        ...
        const addSection = (title, contentFunction, yPos) => {
            ...
            try {
                ...
                addText(title.toUpperCase(), leftMargin, yPos, 12, primaryColor, 'bold');
        ...
        if (personalInfo?.address?.city?.name && personalInfo?.address?.state?.name) {
            contactInfo.push(`Location: ${personalInfo.address.city.name}, ${personalInfo.address.state.name}`);
        }
        if (personalInfo?.address?.pin_code) contactInfo.push(`PIN: ${personalInfo.address.pin_code}`);
        ...
        if (personalInfo?.summary && personalInfo.summary.trim()) {
            yPosition += 5;
            checkPageBreak();
            yPosition = addSection('Summary', (yPos) => {
                return addMultiLineText(personalInfo.summary, leftMargin, yPos, contentWidth);
            }, yPosition);
        }
        ...
        if (experiences.length > 0) {
            checkPageBreak();
            yPosition = addSection('Professional Experience', (yPos) => {
                experiences.forEach((exp, index) => {
                    ...
                    if (exp?.companyName) {
                        addText(exp.companyName, leftMargin, yPos, 10, secondaryColor, 'bold');
                        yPos += 4;
                    }

                    // Date range
                    const startDate = formatDate(exp?.start_date);
                    const endDate = exp?.currently_available ? 'Present' : formatDate(exp?.end_date);
                    addText(`${startDate} - ${endDate}`, leftMargin, yPos, 9, secondaryColor);
                    yPos += 4;

                    if (exp?.industryName) {
                        addText(`Industry: ${exp.industryName}`, leftMargin, yPos, 9, secondaryColor);
                        yPos += 4;
                    }

                    if (exp?.description && exp.description.trim()) {
                        yPos = addMultiLineText(exp.description, leftMargin, yPos, contentWidth);
                        yPos += 2;
                    }

                    // Skills acquired
                    if (exp?.skills_acquired && exp.skills_acquired.length > 0) {
                        const skills = exp.skills_acquired.map(skill => skill?.name).filter(Boolean).join(', ');
                        if (skills) {
                            addText(`Skills: ${skills}`, leftMargin, yPos, 9, secondaryColor);
                            yPos += 4;
                        }
                    }
                });
        ...
        if (educations.length > 0) {
            checkPageBreak();
            yPosition = addSection('Education', (yPos) => {
                educations.forEach((edu, index) => {
                    ...
                    if (edu?.institution) {
                        addText(edu.institution, leftMargin, yPos, 10, secondaryColor, 'bold');
                        yPos += 4;
                    }

                    if (edu?.field_of_studies) {
                        addText(`Field of Study: ${edu.field_of_studies}`, leftMargin, yPos, 9, secondaryColor);
                        yPos += 4;
                    }

                    const startDate = formatDate(edu?.start_date);
                    const endDate = edu?.currently_available ? 'Present' : formatDate(edu?.end_date);
                    addText(`${startDate} - ${endDate}`, leftMargin, yPos, 9, secondaryColor);
                    yPos += 4;

                    if (edu?.skills_acquired && edu.skills_acquired.length > 0) {
                        const skills = edu.skills_acquired.map(skill => skill?.name).filter(Boolean).join(', ');
                        if (skills) {
                            addText(`Skills Acquired: ${skills}`, leftMargin, yPos, 9, secondaryColor);
                            yPos += 4;
                        }
                    }
                });
        ...
        if (projects.length > 0) {
            checkPageBreak();
            yPosition = addSection('Projects', (yPos) => {
                projects.forEach((project, index) => {
                    ...
                    if (project?.company) {
                        addText(`Company: ${project.company}`, leftMargin, yPos, 10, secondaryColor);
                        yPos += 4;
                    }
                    ...
                    if (project?.description && project.description.trim()) {
                        yPos = addMultiLineText(project.description, leftMargin, yPos, contentWidth);
                        yPos += 4;
                    }

                    if (project?.file_url && project.file_url !== 'http://localhost:3000/user/profile') {
                        addText(`Project URL: ${project.file_url}`, leftMargin, yPos, 9, secondaryColor);
                        yPos += 4;
                    }
                });
                return yPos;
        ...
        if (certifications.length > 0) {
            checkPageBreak();
            yPosition = addSection('Certifications', (yPos) => {
                certifications.forEach((cert, index) => {
                    ...
                    if (cert?.issuing_organization) {
                        addText(`Issued by: ${cert.issuing_organization}`, leftMargin, yPos, 10, secondaryColor);
                        yPos += 4;
                    }

                    if (cert?.issue_date) {
                        addText(`Issued: ${formatDate(cert.issue_date)}`, leftMargin, yPos, 9, secondaryColor);
                        yPos += 4;
                    }

                    if (cert?.credential_id) {
                        addText(`Credential ID: ${cert.credential_id}`, leftMargin, yPos, 9, secondaryColor);
                        yPos += 4;
                    }

                    if (cert?.credential_url && cert.credential_url !== 'http://localhost:3000/user/profile') {
                        addText(`URL: ${cert.credential_url}`, leftMargin, yPos, 9, secondaryColor);
                        yPos += 4;
                    }

                    if (cert?.skills_acquired && cert.skills_acquired.length > 0) {
                        const skills = cert.skills_acquired.map(skill => skill?.name).filter(Boolean).join(', ');
                        if (skills) {
                            addText(`Skills: ${skills}`, leftMargin, yPos, 9, secondaryColor);
                            yPos += 4;
                        }
        ...
        if (topSkills.length > 0) {
            checkPageBreak();
            yPosition = addSection('Skills', (yPos) => {
        ...
        addText(`Generated on: ${currentDate}`, rightMargin - 50, footerY, 8, secondaryColor);
...
export const formatDateByMomentTimeZone = (timestamp, format = 'D MMM YYYY') => {
...
export const MediaPreview = ({ url, type, onRemove }) => {
    ...
    return (
        <div className="sticky top-0 left-0 mt-2 max-w-xs border p-2 shadow">
            {type === 'image' && (
                <img src={url} alt="Preview" className="rounded-lg max-h-40 object-cover" />
            )}
            {type === 'video' && (
...
<head>
    ...
    <title>Verification of ${mailData?.companyName ? "Experience" : "Institute"}</title>
    <style type="text/css">
        body, html {
            margin: 0;
            padding: 0;
...
<body>
    <div class="email-container">
        <div class="header">
            <h2>Verification of ${mailData.companyName ? "Experience" : "Institute"}</h2>
        </div>
        <div class="content">
            <p>Dear ${institution.name},</p>
            <p>I am writing to request verification of my ${mailData.companyName ? "Experience" : "Degree"} at your esteemed ${mailData.companyName || mailData?.institution}.</p>
            
            <div class="section-heading">Profile Details</div>
            <div class="detail-item">
                <span class="detail-label">Name:</span> <span class="detail-value">${profileData.username}</span>
            </div>

            <div class="detail-item">
                <span class="detail-label">Dates of Association:</span> <span class="detail-value">${convertTimestampToDate(mailData.start_date)} to ${convertTimestampToDate(mailData?.end_date) || 'Present'}</span>
            </div>
            <div class="detail-item">
                <span class="detail-label">${mailData.companyName || mailData?.institution}</span> <span class="detail-value">:${mailData.profileName || mailData?.degree}</span>
            </div>
